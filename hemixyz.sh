...
if [ "$ARCH" == "x86_64" ]; then
    if [ -d "heminetwork_${VERSION}_linux_amd64" ]; then
        show "Version $VERSION for x86_64 is already downloaded. Skipping download."
        cd "heminetwork_${VERSION}_linux_amd64" || { show "Failed to change directory."; exit 1; }
        download_required=false  # Set flag to false
    fi
elif [ "$ARCH" == "arm64" ]; then
    if [ -d "heminetwork_${VERSION}_linux_arm64" ]; then
        show "Version $VERSION for arm64 is already downloaded. Skipping download."
        cd "heminetwork_${VERSION}_linux_arm64" || { show "Failed to change directory."; exit 1; }
        download_required=false  # Set flag to false
    fi
fi
...
if [ "$download_required" = true ]; then
    if [ "$ARCH" == "x86_64" ]; then
        show "Downloading version $VERSION for x86_64 architecture..."
        wget --quiet --show-progress "https://github.com/hemilabs/heminetwork/releases/download/$VERSION/heminetwork_${VERSION}_linux_amd64.tar.gz" -O "heminetwork_${VERSION}_linux_amd64.tar.gz"
        tar -xzf "heminetwork_${VERSION}_linux_amd64.tar.gz" > /dev/null
        cd "heminetwork_${VERSION}_linux_amd64" || { show "Failed to change directory."; exit 1; }
    elif [ "$ARCH" == "arm64" ]; then
        show "Downloading version $VERSION for arm64 architecture..."
        wget --quiet --show-progress "https://github.com/hemilabs/heminetwork/releases/download/$VERSION/heminetwork_${VERSION}_linux_arm64.tar.gz" -O "heminetwork_${VERSION}_linux_arm64.tar.gz"
        tar -xzf "heminetwork_${VERSION}_linux_arm64.tar.gz" > /dev/null
        cd "heminetwork_${VERSION}_linux_arm64" || { show "Failed to change directory."; exit 1; }
    else
    ...
    show "Skipping download as version $VERSION is already present."
fi
